...
function App() {
  ...
  //initialize audio
  const [playBgSound, { stop: stopBgSound }] = useSound(bgSound, {
    loop: true,
  });
    ...
    if (started) {
      if (window.ethereum) {
        ...
        window.ethereum.enable().then((accounts) => {
          setAccount(accounts[0]);
          const hogwartsAddress = "0xA5a120b2961512338A2fFb5F5F4f0bA49A2DA4e8";
          const randomHouseAddress =
            "0x17923818AFB6a7C038495Ae5Fb28c764faa844d4";
  ...
  const checkNetwork = async () => {
    ...
    if (networkId == 80002) {
      ...
      alert("Please connect to Polygon Amoy Testnet");
    }
  };
  ...
  const getHouseData = async () => {
    setLoading(true);
    const houseIndex = await hogwartsContract.methods
      .getHouseIndex(account)
      .call();
  ...
  const checkNewMinted = async () => {
    ...
    setTimeout(async () => {
      const minted = await hogwartsContract.methods
        .hasMintedNFT(account)
        .call();
